...
def load_data(dataset_name, split_idx=0):
    if dataset_name in ['cora', 'citeseer', 'pubmed']:
        return load_data_citation(dataset_name)
    elif dataset_name in ['ogbn-arxiv']:
        return load_data_ogb(dataset_name)
    elif dataset_name.startswith("geom"):
        return load_data_geom(dataset_name.split('-')[-1], split_idx)
    else:
        raise Exception("The data {} doesn't exist.".format(dataset_name))


def load_data_citation(dataset_name):
...
def load_data_ogb(dataset_name):
...
def load_data_geom(dataset_name, split_idx):

    split = "data/splits/{}_split_0.6_0.2_{}.npz".format(dataset_name, split_idx)
    return load_geom(dataset_name=dataset_name, splits_file_path=split)
...
def load_geom(dataset_name, splits_file_path=None, train_percentage=None, val_percentage=None,
    ...
    if dataset_name in {'cora', 'citeseer', 'pubmed'}:
        adj, features, labels, _, _, _ = load_file_data(dataset_name)
    ...
    if splits_file_path:
        ...
        if dataset_name in {'cora', 'citeseer'}:
            """这里是什么意思，给孤立结点添加连边吗"""
            disconnected_node_file_path = os.path.join('data/unconnected_nodes',
...
def load_file_data(dataset_str):
    """
    ...
    if dataset_str == 'citeseer':
        # Fix citeseer data (there are some isolated nodes in the graph)
